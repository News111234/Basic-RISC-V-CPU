// top/core_top_sim.v (完整修复版 - 修正变量定义问题)
`timescale 1ns/1ps
...
module core_top_sim (
    ...
    //中断调试
    output wire [31:0] debug_csr_inst_pc,      // CSR指令的PC
output wire        debug_csr_inst_valid,   // CSR指令有效
output wire [31:0] debug_csr_inst_instr,   // CSR指令机器码
output wire        debug_csr_write,        // CSR写使能
output wire [11:0] debug_csr_write_addr,   // CSR写地址
output wire [31:0] debug_csr_write_data,   // CSR写数据
output wire [31:0] debug_t0_value,         // t0寄存器值
output wire [31:0] debug_instr_4,          // 指令4的取值
output wire [31:0] debug_instr_5,          // 指令5的取值
output wire [31:0] debug_instr_6,          // 指令6的取值
output wire [31:0] debug_instr_7,           // 指令7的取值

output wire [31:0] debug_ex_csr_result,     // EX阶段的CSR结果
output wire [31:0] debug_ex_mem_csr_result,    // MEM阶段的CSR结果
...
//2
    ...
    // 中断处理单元的输出
    output wire        debug_intr_mepc_we,
    output wire [31:0] debug_intr_mepc_data,
    output wire        debug_intr_mcause_we,
    output wire [31:0] debug_intr_mcause_data,
    output wire        debug_intr_mstatus_we,
    output wire [31:0] debug_intr_mstatus_data,
...
//前递调试信号
    ...
    output wire [4:0]  debug_rs1_addr_id,    // ID阶段的rs1地址
    output wire [4:0]  debug_rs2_addr_id,    // ID阶段的rs2地址
    output wire [31:0] debug_rs1_data_id,    // ID阶段的rs1数据
    output wire [31:0] debug_rs2_data_id,     // ID阶段的rs2数据
...
output wire        debug_id_ex_stall,
 ...
 // ========== 新增流水线控制调试信号 ==========
    // Hazard Unit 调试信号
    output wire        debug_stall_if,           // IF阶段停滞
    output wire        debug_stall_id,           // ID阶段停滞
    output wire        debug_flush_if,           // IF阶段冲刷
    output wire        debug_flush_id,           // ID阶段冲刷
    output wire        debug_load_use_hazard,    // 加载-使用冒险
    output wire        debug_control_hazard,     // 控制冒险
    
    // Hazard Unit 输入调试
    output wire [4:0]  debug_hazard_rs1_addr,    // ID阶段rs1
    output wire [4:0]  debug_hazard_rs2_addr,    // ID阶段rs2
    output wire [4:0]  debug_hazard_ex_rd_addr,  // EX阶段目标寄存器
    output wire        debug_hazard_ex_reg_we,   // EX阶段写使能
    output wire        debug_hazard_ex_mem_re,   // EX阶段是否是load
    
    // Forwarding Unit 调试信号
    output wire [4:0]  debug_fwd_ex_mem_rd,      // EX/MEM阶段目标寄存器
    output wire        debug_fwd_ex_mem_reg_we,  // EX/MEM阶段写使能
    output wire [4:0]  debug_fwd_mem_wb_rd,      // MEM/WB阶段目标寄存器
    output wire        debug_fwd_mem_wb_reg_we,  // MEM/WB阶段写使能
    
    // EX阶段操作数调试
    output wire [31:0] debug_ex_op1,             // ALU操作数1
    output wire [31:0] debug_ex_op2,             // ALU操作数2
    output wire [31:0] debug_ex_rs1_original,    // 原始rs1数据
    output wire [31:0] debug_ex_rs2_original,    // 原始rs2数据
    
    // PC变化调试
    output wire [31:0] debug_next_pc,            // 下一个PC
    output wire        debug_pc_changed,          // PC是否变化

    // ========== GPIO 调试信号 ==========
output wire [31:0] debug_gpio_out,      // GPIO 输出值
output wire [31:0] debug_gpio_oe,       // GPIO 输出使能
output wire [31:0] debug_gpio_in,       // GPIO 输入值
output wire [31:0] debug_gpio_if,       // GPIO 中断标志
output wire        debug_gpio_interrupt,// GPIO 中断输出
...
// ========== Timer 调试信号 ==========
output wire [31:0] debug_timer_load,    // 定时器重装载值
...
wire [31:0] ex_csr_result;      // 来自CSR的结果
...
wire        ex_mem_reg_we;
wire [31:0] ex_mem_csr_result;   // 传递的CSR结果
...
wire [1:0]  mem_wb_sel;
wire        mem_reg_we;
wire [31:0] mem_csr_result;      // 传递的CSR结果
...
wire [1:0]  wb_wb_sel;
wire        wb_reg_we;
wire [31:0] wb_csr_result;       // 传递的CSR结果
wire        mem_wb_mem_re;     // ========== 新增：MEM/WB 阶段的 load 标志 =========
...
// 中断源
wire       intr_software = 1'b0;      // 软件中断（暂不使用）
wire       intr_timer;                 // 定时器中断
wire       intr_external = gpio_interrupt ;   // 合并 GPIO 和 Timer 中断
...
wire       interrupt_handling;
wire [31:0] intr_target_pc;
wire        csr_mepc_we_int;           // 改名避免与常量冲突
wire [31:0] csr_mepc_data_int;
wire        csr_mcause_we_int;
wire [31:0] csr_mcause_data_int;
wire        csr_mstatus_we_int;
wire [31:0] csr_mstatus_data_int;
...
ifu_top u_ifu_top (
    ...
    .jump_target_i    (ex_jump_target),
     ...
     // 新增中断连接
    .interrupt_pending_i(interrupt_taken_pipe),  // 使用 interrupt_taken_pipe
    .mtvec_i           (mtvec),                  // 中断向量地址
...
ex_top u_ex_top (
    ...
    .ex_csr_result_o    (ex_csr_result),
        ...
        // 新增：MEPC 输入
    .csr_mepc_i     (mepc),           // 来自 CSR 的 MEPC
    
    // 新增：MRET 输入（需要从 ID/EX 传递）
    .mret_i         (id_ex_mret)    // 需要在 ID/EX 寄存器中添加
);
...
mem_wb_reg u_mem_wb_reg (
   ...
   // ========== 新增连接 ==========
    .mem_mem_re_i     (ex_mem_mem_re),     // 从 EX/MEM 寄存器来的 load 标志
    .wb_mem_re_o      (mem_wb_mem_re),     // 输出到 forwarding_unit
...
wb_top u_wb_top (
    ...
    .pc_plus4_i     (wb_pc_plus4),
     .csr_result_i   (wb_csr_result),     // 新增：连接CSR结果
    .wb_sel_i       (wb_wb_sel),
    .reg_we_i       (wb_reg_we),
...
gpio u_gpio (
    ...
    .gpio_in_i    (gpio_in_test),      // 外部输入，可接0
...
// ==========================================================================
// 定时器中断生成(250ns后触发一次，用于演示中断功能)
// ==========================================================================
reg [6:0] timer_counter;
reg timer_irq;
reg interrupt_triggered;    // 标记中断是否已触发




assign intr_timer = timer_interrupt;
assign debug_timer_irq = timer_interrupt;  // 输出定时器中断信号到调试端口
assign debug_timer_counter = timer_counter;// 标记中断是否已触发
...
csr_regfile u_csr_regfile (
    .clk_i              (clk_i),
    .rst_n_i            (rst_n_i),
    .csr_addr_i         (id_ex_csr_addr),      // 来自EX阶段的CSR地址
    .csr_rdata_o        (csr_rdata),
    .csr_we_i           (csr_we),               // 合并后的CSR写使能
    .csr_waddr_i        (csr_waddr),            // 合并后的CSR写地址
    .csr_wdata_i        (csr_wdata),            // 合并后的CSR写数据
...
assign csr_rs1_data   = op1_selected;     // 新代码：使用前递后的数据
assign csr_imm        = id_ex_imm;
assign debug_mip      = mip;              // 输出mip寄存器到调试端口
...
// 中断处理单元
// ==========================================================================
interrupt_handler u_interrupt_handler (
    .clk_i              (clk_i),
    .rst_n_i            (rst_n_i),
    .flush_pipeline_i   (interrupt_flush_pipe),
    .interrupt_taken_i  (interrupt_taken_pipe),
    .intr_pending_i     (intr_pending),
    .intr_cause_i       (intr_cause),
    .intr_handler_addr_i(intr_handler_addr),
    .mepc_i             (mepc),
    .mstatus_i          (mstatus),
    .intr_jump_o        (interrupt_taken),
    .intr_target_pc_o   (intr_target_pc),
    .csr_mepc_we_o      (csr_mepc_we_int),      // 改名避免冲突
    .csr_mepc_data_o    (csr_mepc_data_int),
    .csr_mcause_we_o    (csr_mcause_we_int),
    .csr_mcause_data_o  (csr_mcause_data_int),
    .csr_mstatus_we_o   (csr_mstatus_we_int),
    .csr_mstatus_data_o (csr_mstatus_data_int),
    .interrupt_flush_o  (interrupt_flush),
    .interrupt_handling_o(interrupt_handling)
);
...
interrupt_pipeline u_interrupt_pipeline (
    ...
    .rst_n_i            (rst_n_i),
    .if_pc_i            (if_pc),
    .id_valid_i         (|if_id_instr),        // ID阶段指令有效
    .id_pc_i            (if_id_pc),
    .ex_valid_i         (|id_ex_opcode),       // EX阶段指令有效
    .ex_pc_i            (id_ex_pc),
    .ex_branch_taken_i  (ex_branch_taken),
    .ex_jump_taken_i    (ex_jump_taken),
    .mem_valid_i        (ex_mem_mem_we || ex_mem_mem_re),  // MEM阶段有效
    .mem_pc_i           (ex_mem_pc_plus4 - 4), // MEM阶段的PC
    .mem_mem_re_i       (ex_mem_mem_re),
    .mem_mem_we_i       (ex_mem_mem_we),
    .wb_valid_i         (wb_reg_we_out),       // WB阶段有效
    .wb_rd_addr_i       (wb_rd_addr_out),
...
assign final_csr_we    =  csr_mstatus_we_int || csr_inst_we ||
                         pipe_csr_mepc_we || pipe_csr_mcause_we||csr_mepc_we_int;
                         //   csr_mcause_we_int 被禁用

// 优先级：pipeline 的 MEPC 写优先（中断发生时立即写入）
assign final_csr_waddr = pipe_csr_mepc_we ? CSR_MEPC :
                         csr_mepc_we_int ? CSR_MEPC :   // 添加这一行
                         pipe_csr_mcause_we ? CSR_MCAUSE :
                         csr_mcause_we_int ? CSR_MCAUSE :
                         csr_mstatus_we_int ? CSR_MSTATUS :
                         csr_inst_waddr;

assign final_csr_wdata = pipe_csr_mepc_we ? pipe_csr_mepc_data :
                         pipe_csr_mcause_we ? pipe_csr_mcause_data :
                         csr_mepc_we_int ? csr_mepc_data_int :
                         csr_mcause_we_int ? csr_mcause_data_int :
                         csr_mstatus_we_int ? csr_mstatus_data_int :
                         csr_inst_wdata;
...
assign debug_ex_csr_result = ex_csr_result; // 连接CSR结果到EX阶段调试信号
assign debug_ex_mem_csr_result = ex_mem_csr_result; // 连接CSR结果到MEM阶段调试信号
...
// 在 core_top_sim.v 的末尾添加
assign debug_csr_inst_pc = id_ex_pc;

assign debug_csr_inst_instr = {id_ex_opcode, 25'b0}; // 简化
assign debug_csr_write = csr_we;
assign debug_csr_write_addr = csr_waddr;
assign debug_csr_write_data = csr_wdata;
assign debug_t0_value = id_rs1_data; // 假设t0在rs1位置
assign debug_instr_4 = (if_pc == 32'h10) ? if_instr : 32'b0; // 指令4地址0x10
...
assign forward_mem_data = mem_wb_mem_re ? wb_mem_rdata : wb_alu_result;//多路选择
// CSR指令处理模块输入
...
// 中断处理单元输出
assign debug_intr_mepc_we = csr_mepc_we_int;
assign debug_intr_mepc_data = csr_mepc_data_int;
assign debug_intr_mcause_we = csr_mcause_we_int;
assign debug_intr_mcause_data = csr_mcause_data_int;
assign debug_intr_mstatus_we = csr_mstatus_we_int;
assign debug_intr_mstatus_data = csr_mstatus_data_int;
...
// ========== 在 MEM 阶段实例化后添加赋值 ==========
// 在 mem_top_fpga 实例化之后
assign mem_rd_addr_for_fwd = mem_rd_addr;      // 来自 mem_top_fpga 的输出
assign mem_reg_we_for_fwd = mem_reg_we;        // 来自 mem_top_fpga 的输出
assign mem_rdata_for_fwd = mem_bus_rdata;      // 来自总线的读数据
// 流水线控制调试信号
